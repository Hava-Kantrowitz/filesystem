             ...
             off_t offset, struct fuse_file_info *fi)
{
    ...
    printf("WE ARE ENTERING READDIR\n"); 
    ...
    if (rv != 0) {
	    printf("You done goofed in storage stat\n"); 
	    return -ENOENT;
    }
...
{
    //printf("mode is %o\n", mode); 
    //int emptyNum = directory_lookup_empty(); 
    //int rv = directory_put(path, emptyNum);
    printf("IN MKDIR, path is %s\n", path); 
    mode_t myMode = 16877; 
    int rv = storage_mknod(path, myMode); 
    printf("mkdir(%s) -> %d\n", path, rv);
    return rv;
}
...
nufs_symlink(const char *from, const char *to) {
    ...
    printf("WE ARE MAKING A SYM LINK\n"); 
    int rv = storage_syslink(from, to);
    printf("syslink(%s => %s) -> %d\n", from, to, rv);
    return rv;
